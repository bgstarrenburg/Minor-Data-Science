...
columnsToUse = ['Average Income', 'Population', 'Rental vacancy rates', 'Rent prices', 'Housing Units', 'Unemployment', 'Federal Funding' , 'Federal Funding PH']
...
if (False):
    for state in allStates:
        ...
        allScores.append([state, TrainModel(state, X, y, 0.2, 5, False, columnsToUse)])

# create models where data is split by population range 
if (True):
    ranges: list[list[int]] = [[0, 3000000], [3000000, 6000000], [6000000, 9000000], [9000000, 999999999999]]
    for range in ranges:
        mask = (bigX['Population'] > range[0]) & (bigX['Population'] <= range[1])
        X = bigX[mask]
        bigX[~mask]
        bigY[~mask]
        y = bigY[mask]
